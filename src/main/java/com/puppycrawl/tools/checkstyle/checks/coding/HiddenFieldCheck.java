...
/**
 * <p>Checks that a local variable or a pnameter does not shadow
 ...
 * </pre>
 * <p>
 * An example of how to configure the check so that it ignores the pnameter of
 * a setter method is:
 * </p>
 * <pre>
 * &lt;module name="HiddenField"&gt;
 ...
 * <pre>
 * &lt;module name="HiddenField"&gt;
 *    &lt;property name="ignoreConstructorPnameter" value="true"/&gt;
 * &lt;/module&gt;
 * </pre>
 * @author Rick Giles
 * @version 1.0
    ...
    extends Check
{
    ...
    private void processVariable(DetailAST ast)
    {
        ...
        if ((currentFrame.containsStaticField(name)
            ...
            && !isIgnoredParaofAbstractMethod(ast))
        {
    ...
    /**
     * Decides whether to ignore an AST node that is the pnameter of a
     * setter method, where the property setter method for field 'xyz' has
     * name 'setXyz', one pnameter named 'xyz', and return type void.
     * @param ast the AST to check.
     * @param name the name of ast.
     * @return true if ast should be ignored because check property
     * ignoreSetter is true and ast is the pnameter of a setter method.
     */
    private boolean isIgnoredSetterParam(DetailAST ast, String name)
    {
        ...
        //single pnameter?
        final DetailAST pnametersAST = ast.getParent();
        if (pnametersAST.getChildCount() != 1) {
            return false;
        }
        //method pnameter, not constructor pnameter?
        final DetailAST methodAST = pnametersAST.getParent();
     ...
     * @param ast the AST to check.
     * @return true if ast should be ignored because check property
     * ignoreConstructorPnameter is true and ast is a constructor pnameter.
     */
    private boolean isIgnoredConstructorParam(DetailAST ast)
    {
        ...
        final DetailAST pnametersAST = ast.getParent();
        final DetailAST constructorAST = pnametersAST.getParent();
        return (constructorAST.getType() == TokenTypes.CTOR_DEF);
    }

    /**
     * Decides whether to ignore an AST node that is the pnameter of an
     * abstract method.
     * @param ast the AST to check.
     * @return true if ast should be ignored because check property
     * ignoreAbstactMethods is true and ast is a pnameter of abstract
     * methods.
     */
    private boolean isIgnoredParaofAbstractMethod(DetailAST ast)
    {
        if ((ast.getType() != TokenTypes.PARAMETER_DEF)
    ...
    /**
     * Set whether to ignore the pnameter of a property setter method.
     * @param ignoreSetter decide whether to ignore the pnameter of
    ...
    /**
     * Set whether to ignore constructor pnameters.
     * @param ignoreConstructorPnameter decide whether to ignore
     * constructor pnameters.
     */
    ...
    /**
     * Set whether to ignore pnameters of abstract methods.
     * @param ignoreAbstractMethods decide whether to ignore
     * pnameters of abstract methods.
     */
