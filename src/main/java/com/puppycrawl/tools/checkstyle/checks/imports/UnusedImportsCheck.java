...
public class UnusedImportsCheck extends Check
{
    ...
    /**
     * Collects references made in Javdoc comments.
     * @param ast node to inspect for Javdoc
     */
    private void processJavdoc(DetailAST ast)
    {
        final FileContents contents = getFileContents();
        final int lineNo = ast.getLineNo();
        final TextBlock cmt = contents.getJavadocBefore(lineNo);
        if (cmt != null) {
            referenced.addAll(processJavdoc(cmt));
        }
    }

    /**
     * Process a javdoc {@link TextBlock} and return the set of classes
     * referenced within.
     * @param cmt The javdoc block to parse
     * @return a set of classes referenced in the javdoc block
     */
    private Set<String> processJavdoc(TextBlock cmt)
    {
                ...
                : getValidTags(cmt, JavadocUtils.JavadocTagType.INLINE))
        {
            if (tag.canReferenceImports()) {
                references.addAll(processJavdocTag(tag));
            }
        }
    ...
    /**
     * Returns the list of valid tags found in a javdoc {@link TextBlock}
     * @param cmt The javdoc block to parse
    ...
    /**
     * Returns a list of references found in a javdoc {@link JavadocTag}
     * @param tag The javdoc tag to parse
     * @return A list of references found in this tag
     */
    private Set<String> processJavdocTag(JavadocTag tag)
    {
        final Set<String> references = new HashSet<String>();
