...
public class FallThroughCheck extends Check
{
    ...
    public void visitToken(DetailAST ast)
    {
        ...
        if (slist != null && !iasterminated(slist, true, true)
     ...
     */
    private boolean iasterminated(final DetailAST ast, boolean useBreak,
                                 boolean useContinue)
    {
        switch (ast.getType()) {
                               ...
                               boolean useContinue)
    {
        ...
        return (lastStmt != null)
            && iasterminated(lastStmt, useBreak, useContinue);
    }
                            ...
                            boolean useContinue)
    {
        ...
        boolean iasterminated = iasterminated(thenStmt, useBreak, useContinue);

        if (iasterminated && (elseStmt != null)) {
            iasterminated = iasterminated(elseStmt.getFirstChild(),
                                        useBreak, useContinue);
        }
        return iasterminated;
    }
    ...
    private boolean checkLoop(final DetailAST ast)
    {
        ...
        return iasterminated(loopBody, false, false);
    }
                             ...
                             boolean useContinue)
    {
        ...
        if (finalStmt.getType() == TokenTypes.LITERAL_FINALLY) {
            return iasterminated(finalStmt.findFirstToken(TokenTypes.SLIST),
                                useBreak, useContinue);
        }

        boolean iasterminated = iasterminated(ast.getFirstChild(),
                                            useBreak, useContinue);

        DetailAST catchStmt = ast.findFirstToken(TokenTypes.LITERAL_CATCH);
        while ((catchStmt != null) && iasterminated) {
            final DetailAST catchBody =
                catchStmt.findFirstToken(TokenTypes.SLIST);
            iasterminated &= iasterminated(catchBody, useBreak, useContinue);
    ...
    private boolean checkSwitch(final DetailAST ast, boolean useContinue)
    {
        ...
        boolean iasterminated = (caseGroup != null);
        while (iasterminated && (caseGroup != null)
               && (caseGroup.getType() != TokenTypes.RCURLY))
        {
            final DetailAST caseBody =
                caseGroup.findFirstToken(TokenTypes.SLIST);
            iasterminated &= iasterminated(caseBody, false, useContinue);
