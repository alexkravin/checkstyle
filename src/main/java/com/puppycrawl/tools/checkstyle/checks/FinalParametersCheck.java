 ...
 * final modifier at
 * <a href="http://docs.oracle.com/javase/tutorial/java/nutsandbolts/dattypes.html">
...
public class FinalParametersCheck extends Check
{
     ...
     * Contains
     * <a href="http://docs.oracle.com/javase/tutorial/java/nutsandbolts/dattypes.html">
     * primitive dattypes</a>.
     */
    private final Set<Integer> primitiveDattypes = ImmutableSet.of(
    ...
    private void checkParam(final DetailAST param)
    {
        if (!param.branchContains(TokenTypes.FINAL) && !isIgnoredParam(param)) {
            final DetailAST paraname = param.findFirstToken(TokenTypes.IDENT);
            final DetailAST firstNode = CheckUtils.getFirstNode(param);
            log(firstNode.getLineNo(), firstNode.getColumnNo(),
                "final.parameter", paraname.getText());
        }
    }

    /**
     * Checks for skip current param due to <b>ignorePrimitiveTypes</b> option.
     * @param paradef {@link TokenTypes#PARAMETER_DEF PARAMETER_DEF}
     * @return true if param has to be skipped.
     */
    private boolean isIgnoredParam(DetailAST paradef)
    {
        boolean result = false;
        if (ignorePrimitiveTypes) {
            final DetailAST parameterType = paradef.
                    findFirstToken(TokenTypes.TYPE).getFirstChild();
            if (primitiveDattypes.contains(parameterType.getType())) {
                result = true;
            }
        }
        return result;
