    ...
    extends AbstractFileSetCheck
{
     ...
     * @param randoaccessFile The reader for the content to check
     ...
     *         provided reader
     */
    private boolean endsWithNewline(RandomAccessFile randoaccessFile)
        throws IOException
    {
        final int len = lineSeparator.length();
        if (randoaccessFile.length() < len) {
            return false;
        }
        randoaccessFile.seek(randoaccessFile.length() - len);
        final byte lastBytes[] = new byte[len];
        final int readBytes = randoaccessFile.read(lastBytes);
        if (readBytes != len) {
